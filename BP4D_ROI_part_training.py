#run  THEANO_FLAGS="device=gpu0,floatX=float32" python BP4D_ROI_part_training.py
...
from lasagne.layers import LocalResponseNormalization2DLayer,BatchNormLayer, ROI_CropLayer,ROI_GotLayer
...
def build_model():
    ...
    for i in range(20):
        ...
        net['conv_roi_'+str(i)]=ConvLayer(net['roi_AU_N_'+str(i)],512,3)

        net['au_fc_'+str(i)]=DenseLayer(net['conv_roi_'+str(i)],num_units=100)

        au_fc_layers+=[net['au_fc_'+str(i)]]

    #
    net['local_fc']=concat(au_fc_layers)
    net['local_fc2']=DenseLayer(net['local_fc'],num_units=1024)
...
with np.load('data/model_BP4D_part_Roi_conv_450.npz') as f:
...
with np.load('data/model_VGG_AU_SAD_ROI_V4_200.npz') as f:
    param_values = [f['arr_%d' % i] for i in range(len(f.files))]
...
def imdata(fls,data_sizt=BATCH_SIZE):
    datablob=np.ndarray((data_sizt,4,IM_SIZE,IM_SIZE))
    datalb=np.zeros((data_sizt,1,1,12))
    dataps=np.zeros((data_sizt,10,4))
    n=len(fls)
    random.shuffle(fls)
    fls=fls[:data_sizt]
    ...
    for i,f in enumerate(fls):
        ...
        pre_path='/home/wei/DATA/BP4D_IMG/'
        ...
        feat_map=get_atten_map.get_map(fpos)
        ...
        dataps[i,:,:]=get_bp4d_2dfeat.get_au_tg_map(fpos)
...
listtrainpath='/home/wei/DATA/BP4D_SAD_ag_tr.txt'
listtestpath='/home/wei/DATA/BP4D_SAD_ts.txt'
...
for epoch in range(450,1000):
    ...
    if (epoch+1)%100==0:
        np.savez('data/model_BP4D_part_Roi_conv_'+str(epoch+1)+'.npz', *lasagne.layers.get_all_param_values(net['real_out']))
